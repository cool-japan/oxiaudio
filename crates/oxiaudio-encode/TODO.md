...
- [~] CELT mode with PVQ band-shape coding: MDCT (OxiFFT 960-pt), 21-band energy quantization (4-bit, 16 levels, 60 dB range), greedy PVQ pulse allocator wired to exact CWRS encode_pulses (opus_pvq.rs), encode_celt_frame_pvq standalone API, 24-test suite — range coder now bit-exact with RFC 6716 ec_enc (carry-buffer, end-packed raw bits); CWRS icwrs/encode_pulses verified by final-range equality; large-band V(N,K) overflow guard implemented (u64 wide path) (~400 SLOC)
